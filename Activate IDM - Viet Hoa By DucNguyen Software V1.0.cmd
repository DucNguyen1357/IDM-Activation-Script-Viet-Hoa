...
%nceline%
echo Da phat hien phien ban he dieu hanh khong duoc ho tro.
echo Du an chi duoc ho tro cho Windows 7/8/8.1/10/11 va Server tuong duong.
goto done2
)
...
%nceline%
echo Script nay yeu cau dac quyen cua quan tri vien.
echo De lam nhu vay, hay nhap chuot phai vao Script va chon 'Chay voi tu cach quan tri vien'.
goto done2
)
...
cls
title IAS 0.7 (Fork of @Dukun Cabul's Tool) Viet hoa By DucNguyen Software
mode 85, 25
...
if %_dis%==3 (
set _status=Vo_hieu_hoa
set _col=%_Red%
)

if not %_ena%==3 if not %_dis%==3 (
set _status=Khong_ro
set _col=%_Yellow%
)
...
echo:                  [2] Dat lai kich hoat IDM / Dung thu trong Registry
echo:                ________________________________________________________   
echo:                                                           
call :_color2 %_White% "                  [3] Chuyen Tuong lua cua Windows  " %_col% "[%_status%]"
echo:                ________________________________________________________   
echo:                                                               
echo:                  [4] ReadMe                                      
echo:                  [5] Trang chu                                    
...
echo:   
call :_color2 %_White% "                   " %_Green% "Nhap mot tuy chon menu trong ban phim [1,2,3,4,5,6]"
choice /C:123456 /N
set _erl=%errorlevel%
...
call :_color %Green% "IDM Activation - Ban dung thu duoc dat lai thanh cong trong registry."
) else (
call :_color %Red% "Khong the dat lai hoan toan IDM Activation - Ban dung thu."
)
...
:_activate
cls
call :_color %_Yellow% "Them ten tuy chinh vao Infomation License IDM"
call :_color %_Yellow% "Ten co the duoc viet bang tieng anh hoac tieng viet khong dau hoac so o duoi"
...
echo Ban co the tai xuong tu  https://www.internetdownloadmanager.com/download.html
goto done
)
...
echo:
call :_color %Gray% "Neu man hinh noi tiep gia xuat hien, hay chay lai acitve, sau do no se khong xuat hien."
goto done
)
...
echo %line%
echo:
call :_color %Red% "Da tim thay loi, dang dat lai IDM Activation..."
set "_action=call :delete_key"
call :reset
echo:
echo %line%
echo:
call :_color %Red% "Khong the kich hoat IDM."
exit /b
...
cls
echo:
echo Kich hoat mot vai lan tai xuong de tao mot so registry keys nhat dinh, vui long doi...
...
echo Cac registry keys bat buoc da duoc tao thanh cong.
) else (
if not defined _fileexist call :_color %Red% "Khong the tai xuong tep bang IDM."
call :_color %Red% "Khong tao duoc registry keys bat buoc."
call :_color %Magenta% "Thu lai - vo hieu hoa Windows Firewall bang menu - chon ReadMe."
)
...
_________________________________
 ...
 - Script nay ap dung phuong phap khoa registry de kich hoat Internet download manager (IDM).

 - Phuong phap nay yeu cau Internet tai thoi diem kich hoat.

 - Cac ban cap nhat IDM co the duoc cai dat truc tiep ma khong can phai kich hoat lai.

 - Sau khi kich hoat, trong mot so truong hop, IDM bat dau hien thi man hinh kich hoat,
   sau do chi can chay lai Activation.

_________________________________

   Dat lai kich hoat IDM / Dung thu:
_________________________________

 - Internet download manager cap thoi gian dung thu 30 ngay, ban co the su dung Script nay
   de reset thoi gian kich hoat / dung the bat cu khi nao ban muon.
 
 - Tuy chon nay cung co the duoc su dung de khoi phuc trang thai neu trong truong hop
   IDM bao cao Serial gia va cac loi tuong tu khac.

_________________________________

   Yeu cau he dieu hanh:
_________________________________

 - Du an chi duoc ho tro cho Windows 7/8/8.1/10/11 va Server tuong duong.
...
_________________________________
   ...
   - De kich hoat o che do khong giam sat.
   - De dat lai o che do khong giam sat, hay chay Script voi tham so / res0.
   - De bat che do im lang voi hai phuong phap tren, hay chay Script voi tham so /s.

Cac gia tri co the duoc chap nhan,
...
_________________________________
 ...
 - Cac buoc khac phuc su co:
_________________________________

   - Neu bat ky trinh kich hoat nao khac da duoc su dung de kich hoat IDM truoc do thi hay dam bao
     go cai dat dung cach voi trinh kich hoat do (neu co tuy chon), dieu nay dac biet quan trong
     neu su dung bat ky phuong phap chan tuong lua / Registry.

   - Go cai dat IDM khoi Control Panel.

   - Dam bao thiet lap IDM goc moi nhat duoc su dung de cai dat, ban co the tai xuong tu
     https://www.internetdownloadmanager.com/download.html

   - Bay gio cai dat IDM va su dung tuy chon kich hoat trong Script nay va neu khong thanh cong thi : 

     - Vo hieu hoa Windows Firewall voi Script, dieu nay giup ich trong truong hop cac muc nhap con
       sot lai cua trinh kich hoat da su dung truoc do (mot so phuong phap va tep cung tao ra cac
       Firewall entries).

     - Mot so chuong trinh Security co the chan Script nay, dieu nay la sai, mien la ban da tai xuong
       tu bai dang goc (duoc de cap ben duoi trong trang nay), tam ngung Security Antivirus trong thoi
       gian thuc hoac loai tru tep da tai xuong / thu muc duoc trich xuat khoi qua trinh quet.

     - Neu ban van gap bat ky van de nao, vui long lien he voi toi (duoc de cap ben duoi trong trang nay).
...
____________________________________________________________________________________________________
   ...
   @Dukun Cabul        - Nha nghien cuu ban dau cua Logic kich hoat va thiep lap lai 
                         ban dung thu IDM nay, da tao ra mot cong cu Autoit cho cac 
                         phuong phap nay, IDM-AIO_2020_Final
                         nsaneforums.com/topic/371047--/?do=findComment&comment=1632062
                         
   @WindowsAddict (Me) - Da chuyen cong cu Autoit o tren sang Script hang loat

   @AveYo aka @BAU     - Doan ma de dat quyen so huu va quyen dang ky mot cach de quy
                         pastebin.com/XTPt0JSC

   @abbodi1406         - Tro giup va thu thuat Script hang loat 

   @dbenham            - Dat Buffer Height doc lap voi chieu cao cua so
                         stackoverflow.com/a/13351373
   
   @DucNguyen1357      - Viet hoa Script, chinh sua mot so dong lenh
                         Cam on Dukun Cabul cung nhu moi nguoi da viet Script nay
                         Thanks to Dukun Cabul as well as everyone who wrote this amazing Script
                         ducnguyen.x10.bz/ducnguyensoftware
